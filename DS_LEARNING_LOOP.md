...
| Timeline connecting line uses `inset-inline-start: 15px` (standard) or `calc(50% - 1px)` (alternating) without translateX | Centers the 2px line by subtracting half its width from the position. `translateX(-50%)` was removed because it is a physical-direction transform that breaks RTL alignment. Logical properties alone handle both LTR and RTL. | Timeline, any vertically-connected layout |
